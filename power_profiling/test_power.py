...
orig_names = ['fpga_0.75', 'fpga_1.0', 'fpga2_0.5', 'fpga2_0.35', 'fpga2_0.75',
               'fpga2_1.0', 'fpga2_1.0_224', 'fpga2_1.0_new', 'mbv2_0.5', 'mbv2_0.35',
               'mbv2_0.75', 'mbv2_1.0']

# names dictionary: kernel_name : orig_name
kernel_names = [string.replace('_', '').replace('.', '') for string in orig_names]
get_orig_name = dict()
for i in range(len(orig_names)):
    get_orig_name[kernel_names[i] + '_0'] = orig_names[i]
...
def run(so_name='libdpumodelfpga10.so', time=1000):
    ...
    cmd = latency_binary + " " + kernel_name + " " + str(time) + " > " + kernel_name + ".txt"
    print(cmd)
    start = datetime.now()
    os.system(cmd)
    end = datetime.now()
...
def run_power_profiling():
    ...
    for kernel in kernels:
        ...
        print("waiting...")
        sleep(5)
        net_name = get_orig_name[kernel_name]
